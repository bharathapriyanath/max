...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to max voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
max.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('pinky') && conf.TALKING_PINKY !== 'true') {        
        var unique_ident = message.client.user.jid.split('@')[0]      
        let acc = os.userInfo().homedir.split('Whats')[1].split('Duplicated/')[0] == 'max' ? '7d57838203msh0c5cf65c90a7231p13b461jsn77c8cfa55871' : '7d57838203msh0c582jak19865261js1229n77c8cfa55871'
...
max.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
...
max.addCommand({ pattern: 'vtalk$', desc: vtalk_dsc,dontAddCommandList: true, fromMe: wk }, (async (message, match) => {
...
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional max features. Turn your account into a ai chatbot!'
    already_on = 'max artificial intelligence is already fully functional.'
    already_off = 'max artificial intelligence is currently running semi-functional.'
    succ_on = 'max Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'max Set to Semi-Functional! Please wait a bit! ☑️'
}
...
max.addCommand({ pattern: 'talkmax ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.pinky on / off' }, (async (message, match) => {
